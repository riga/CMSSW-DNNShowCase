...
td, inp, outp = None, None, None\n\
def setup(python_path, model_file, inp_name, outp_name):\n\
    global td, inp, outp\n\
...
DNNShowCase::DNNShowCase(const edm::ParameterSet& iConfig)
{
    ...
    std::string tfdeployBase = cmsswBase + "/python/DNNShowCase/DNNShowCase";
    ...
    PyTuple_SetItem(pyArgs, 1, PyString_FromString(_modelFile.c_str()));
...
double DNNShowCase::evalModel(const pat::Jet& jet)
{
    ...
    PyTuple_SetItem(_pyEvalArgs, idx++, PyFloat_FromDouble(jet.pt()));
    PyTuple_SetItem(_pyEvalArgs, idx++, PyFloat_FromDouble(abs(jet.eta())));
    PyTuple_SetItem(_pyEvalArgs, idx++, PyFloat_FromDouble(jet.nConstituents()));
